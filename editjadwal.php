<?php
 	 ...
 	 ?>
 	  ...
 	  $uid = $_GET['uid'];
 	  $sql = mysqli_query($con, "select * from jadwal inner join users on users.u_id = jadwal.j_th where j_id = '$uid'") or die (mysqli_error());
  	$data = mysqli_fetch_assoc($sql);
    $sql = mysqli_query($con, "select * from jadwal inner join users on users.u_id = jadwal.j_ta where j_id = '$uid'") or die (mysqli_error());
...
<div class="row" id="syarat2">
  	...
  	<div class="col-md-4 animate fadeIn" style="margin-top: 0; border: 1px solid white;">
          ...
          <input type="text" id="nama" name="uid" value="<?php echo "$uid"; ?>" hidden>	
	      	<p>Home</p>                          
	        <input type="text" id="nama" name="p_nama" value="<?php echo "$data[u_nama]"; ?>" readonly>     	
	      	<p>Away</p>                        
	        <input type="text" id="username" name="p_nrp" value="<?php echo "$data2[u_nama]"; ?>" readonly>
	        <p>Waktu Pertandingan</p>              
	        <input type="text" id="username" name="p_nopung" value="<?php echo "$data[j_tanggal]"; ?>" readonly>
          <p>Score Home:</p>              
          <input type="text" id="username" name="j_th" >
          <p>Score Away:</p>              
          <input type="text" id="username" name="j_ta" >
